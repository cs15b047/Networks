...
static int parse_packet(struct sockaddr_in *src, struct sockaddr_in *dst,
                        int64_t **payload, size_t *payload_len,
                        struct rte_mbuf *pkt) {
    ...
    if (!rte_is_same_ether_addr(&mac_addr, &eth_hdr->dst_addr)) {
        ...
        printf("Packet MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8
               " %02" PRIx8 " %02" PRIx8 "\n",
               eth_hdr->dst_addr.addr_bytes[0], eth_hdr->dst_addr.addr_bytes[1],
               eth_hdr->dst_addr.addr_bytes[2], eth_hdr->dst_addr.addr_bytes[3],
               eth_hdr->dst_addr.addr_bytes[4],
               eth_hdr->dst_addr.addr_bytes[5]);
        printf("My MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8
               " %02" PRIx8 " %02" PRIx8 "\n",
...
static void set_eth_hdrs(struct rte_ether_hdr *eth_hdr,
...
static void set_ipv4_hdrs(struct rte_ipv4_hdr *ipv4_hdr, rte_be32_t src_addr,
                          rte_be32_t dst_addr, size_t pkt_len) {
    ipv4_hdr->version_ihl = 0x45;
    ipv4_hdr->type_of_service = 0x0;
    ipv4_hdr->total_length = rte_cpu_to_be_16(
        sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_tcp_hdr) + pkt_len);
...
static void set_tcp_request_hdrs(struct rte_tcp_hdr *tcp_hdr,
    ...
    tcp_hdr->rx_win = rte_cpu_to_be_16(
        TCP_WINDOW_LEN *
        packet_len); // window size = # of packets * packet length
    tcp_hdr->sent_seq =
        rte_cpu_to_be_32(seq_num); // seq[flow] denotes sequence number per flow
...
static void set_tcp_response_hdrs(struct rte_tcp_hdr *tcp_hdr,
    ...
    tcp_hdr->recv_ack =
        rte_cpu_to_be_32(ack_seq); // Acknowledgement number is sequence number
                                   // of the packet ack'd
...
static void set_payload(uint8_t *ptr, struct rte_mbuf *pkt, size_t pkt_len,
                        size_t header_size, int64_t *data) {
    /* set the payload */
