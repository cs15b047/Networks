...
void process_data(struct rte_ether_hdr *eth_h,
    ...
    if (new_worker) {
        ...
        printf("From Worker %d, expected %ld elements\n", partition_idx,
               partition_info->data_len);
    } else {
        ...
        if(elements_recvd > partition_info->data_len) {
            printf("Error: elements_recvd %ld > data_len %ld\n", elements_recvd, partition_info->data_len);
            return;
        }
...
static void WorkerStart() {
    ...
    thread send_thread = thread([&]() { 
        for(int i = 0; i < num_workers; i++) {
            if (i != own_rank) {
                send_data(own_partition_data.data, &worker_macs[i], own_rank);
            }
        }
    });
    thread recv_thread = thread([&] () {
        receive_data();
    });
    send_thread.join();
    recv_thread.join();
