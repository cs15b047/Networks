...
void send_partition(vector<int>& partition, int partition_size) {
...
vector<int> receive_partitions(int num_workers, vector<int>& merged_arr, size_t recv_ptr) {
    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
    struct sockaddr_in serv_addr = get_server_info();
...
int main(int argc, char *argv[]) {
    ...
    if(rank != 0){
        send_partition(partition, partition_size);
    } else {
