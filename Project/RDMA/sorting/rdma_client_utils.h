...
static struct rdma_event_channel *cm_event_channel_client = NULL;
...
static int client_prepare_connection(struct sockaddr_in *s_addr)
{
	...
	cm_event_channel_client = rdma_create_event_channel();
	if (!cm_event_channel_client) {
	 ...
	 */
	ret = rdma_create_id(cm_event_channel_client, &cm_client_id, 
			NULL,
			RDMA_PS_TCP);
	if (ret) {
	...
	ret  = process_rdma_cm_event(cm_event_channel_client, 
 ...
 * purposes
 */
static int client_xchange_metadata_with_server(char* src, uint32_t src_len_bytes)
{
	struct ibv_wc wc[2];
	int ret = -1;
...
static int client_disconnect_and_clean()
{
	...
	rdma_destroy_event_channel(cm_event_channel_client);
